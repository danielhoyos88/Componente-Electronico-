...
{
    public class FormListPassive : Form
    {
        private readonly ApiService _api = new();
        private TextBox txtBrand, txtPackage;
        private DataGridView grid;

        public FormListPassive()
        {
            Text = "Listar Componentes Pasivos";
        ...
        private async System.Threading.Tasks.Task LoadData()
        {
            try
            {
                ...
                List<PassiveComponent> list = await _api.GetAllPassiveAsync(brand, pkg);

                grid.DataSource = list.Count > 0 ? list : new List<PassiveComponent>();
                ...
                if (grid.Columns.Count > 0)
                {
                    ...
                    grid.Columns["Tolerance"].HeaderText = "Tolerancia (%)";
                    grid.Columns["NominalMagnitude"].HeaderText = "Magnitud";
                    grid.Columns["NominalUnit"].HeaderText = "Unidad";
